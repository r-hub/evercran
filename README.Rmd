...
0. Add your public SSH key to GitHub, at https://github.com/settings/keys.
   If you don't have an SSH key-pair, generate one first using your
   SSH client.
...
11. Copy the SSH command below and paste it into a terminal window.
    Or, if you use an SSH app, copy the connection URL and use it in
	the app. You'll need your SSH key to authenticate.

R is running in a Docker container, in a tmate session.
To open a new shell in the container use `CTRL+b c` (press `CTRL`
+ `b` together, let them go, then press `c`). Type `R` in the shell
to open another R session. Use `CTRL+b n` to switch between your
shells.

When you quit from all shells, the container also stops and the
workflow terminates.
